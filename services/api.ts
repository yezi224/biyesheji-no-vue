...
const fetchApi = async (url: string, options: RequestInit = {}) => {
    ...
    try {
        // 'credentials: "include"' is crucial for sending session cookies across different ports (origins).
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (response.status === 401) {
                    errorMessage = '认证失败，请检查您的登录状态或权限。';
                }
            }
...
export const ApiService = {
    ...
    resetPassword: (userId: number, password: string): Promise<{ success: boolean }> =>
        fetchApi(`${API_BASE_URL}/users/${userId}/password`, {
            method: 'PUT',
